...
const Widgets = ({ newsResults, randomUsersResults }) => {
  ...
  return (
    <div className="xl:w-[600px] hidden lg:block ml-8 space-y-5">
      <div className="sticky top-0 w-[90%] xl:w-[75%] bg-white py-1.5 z-50">
      ...
      <div className="sticky top-16 text-gray-700 space-y-3 bg-gray-100 pt-2 rounded-xl w-[90%] xl:w-[75%]">
        <h4 className="font-bold text-xl px-4">Who to follow</h4>
          {randomUsersResults.slice(0, randomUserNum).map((randomUser) => (
            ...
            <picture>
                <source srcSet={randomUser.picture.thumbnail}  type="image/webp" />
                <img src={randomUser.picture.thumbnail} alt={randomUser.login.username} width="40" className="rounded-full" />
            </picture>
            <div className="truncate ml-4 leading-5">
              <h4 className="font-semibold hover:underline text-[14px] truncate">{randomUser.login.username}</h4>
